...
function esc(v) { // escape HTML, щоб не було XSS/поламаного HTML
  return String(v ?? "")
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function shortItemId(itemId) { // скорочує itemId виду v1|1105...|0 -> 1105...
  if (!itemId) return "";
...
function money(value, currency) { // формат "value currency" або "—"
  const v = value ?? "";
  const c = currency ?? "";
  const s = `${v} ${c}`.trim();
  return s || "—";
...
function toNumber(x) { // safe Number(), якщо NaN - 0
  const n = Number(x);
  return Number.isFinite(n) ? n : 0;
}

function fmtNum(x, digits = 2) { // формат числа для аналітики
...
function renderAnalytics(boxEl, payload) { // рендерить аналітику
  ...
  boxEl.style.display = "flex"; // показати блок
}

/* Charts */
let _priceHistChart = null; // графіка гістограми total
let _countryChart = null;   // графіка топ-країн

function destroyCharts() { // безпечно знищує графіки перед новим рендером
...
function buildHistogram(values, bins = 10) { // будує біни гістограми (labels + counts)
  const xs = values.filter((v) => Number.isFinite(v));
  if (!xs.length) return { labels: [], counts: [] };

  let min = Math.min(...xs);
  let max = Math.max(...xs);
  if (min === max) { min -= 1; max += 1; } // щоб не було step=0
  ...
  const labels = counts.map((_, i) => { // підписи бінів
    const a = min + i * step;
...
function buildCountryTop(items, topN = 10) { // топ країн за кількістю items
  const map = new Map();
...
function renderCharts(items) { // рендерить 2 графіки: total histogram + top countries
  ...
  if (typeof Chart === "undefined") return; // chart.js не підвантажився

  destroyCharts(); // щоб не було дублю

  const totals = items.map((it) => toNumber(it.price_value) + toNumber(it.shipping_value)); // total=price+ship
  const hist = buildHistogram(totals, 10); // гістограма
  const country = buildCountryTop(items, 10); // топ країн

  if (!hist.labels.length || !country.labels.length) { // якщо нема даних
    chartsBox.style.display = "none";
    return;
  }

  chartsBox.style.display = "grid"; // показати блок графіків

  const commonOptions = {
    responsive: true,
    maintainAspectRatio: false, // якщо контейнер має фіксовану висоту
    plugins: { legend: { display: false } },
  };

  _priceHistChart = new Chart(priceCanvas.getContext("2d"), { // графік total histogram
    type: "bar",
    data: {
      labels: hist.labels,
  ...
  _countryChart = new Chart(countryCanvas.getContext("2d"), { // графік top countries
    type: "bar",
    data: {
...
document.addEventListener("DOMContentLoaded", () => { // старт після завантаження
  ...
  let lastParams = null; // останні параметри для export

  if (!form) return; // якщо форми нема - вихід

  function buildParams() { // збирає params для /api/search та /api/analytics
  ...
  async function loadAnalytics(params) { // тягне /api/analytics і рендерить карточки
  ...
  if (exportBtn) { // кнопка експорту Excel
    exportBtn.addEventListener("click", () => {
      if (!lastParams) return;
      const url = "/api/export?" + lastParams.toString();
      window.location.href = url; // завантаження файлу
    });
  }

  form.addEventListener("submit", async (e) => { // обробка пошуку
    e.preventDefault();
    ...
    const params = buildParams(); // зібрати params
    ...
    try {
      ...
      metaLine.textContent = `Знайдено: ${data.total ?? 0}. Показано: ${items.length}`; // коротка мета-інформація
      ...
      table.style.display = "table"; // показати таблицю
      lastParams = params; // зберегти params для export
      if (exportBtn) exportBtn.disabled = false;

      await loadAnalytics(params); // підтягнути карточки аналітики
      renderCharts(items); // намалювати графіки
      ...
      if (loadingBox) loadingBox.style.display = "none"; // сховати loader
    }
  });
});
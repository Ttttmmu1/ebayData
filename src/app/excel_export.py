...
def build_excel(
    *,
    ...
    for idx, it in enumerate(items or [], start=1):
        ...
        cur = (it.get("price_currency") or it.get("shipping_currency") or "") or None
        total_val = (pv or 0.0) + (sv or 0.0) if (pv is not None or sv is not None) else None
    ...
    # -------- Sheet 2: Analytics (extended)
    ws2 = wb.create_sheet("Analytics")

    analytics = compute_analytics(items or [])
    cur = analytics.get("currency_most_common") or ""
    ...
    # -------- Sheet 3: Charts (with data + charts)
    ...
    countries = (analytics.get("top", {}).get("countries") or [])[:7]
    for e in countries:
    ...
    bins = (analytics.get("hist", {}).get("total", {}).get("bins") or [])
    for b in bins:
        ws3.append([b.get("from"), b.get("to"), b.get("count")])

    # Histogram-like bar chart (count by bin index)
    if bins:
        # Add helper labels (bin as "a-b")
        label_col = 4
        ...
        ws3.add_chart(hist_chart, f"D{start_row+2}")
